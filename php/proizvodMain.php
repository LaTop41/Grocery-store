...
if(isset($_POST['idInfo']) || isset($_SESSION['idIzabranog'])){
	if(isset($_POST['idInfo'])){
	$idIzabrani = $_POST['idInfo'];
	}
	else{
		$idIzabrani = $_SESSION['idIzabranog']; 
	}

	$upit = "SELECT * FROM proizvod WHERE idProizvoda = '" .$idIzabrani. "'";

	$rezultat = $mysqli->query($upit) or die("Greska: " . $mysqli->error );
...
<div class="row">
    <div class="col-md-6">
        <img class="slikaProizvoda" src="<?php  echo "img/". $proiz['slika'];?>">
    </div>
    <div class="col-md-6">
        <div class="row">
            <div>
                ...
                <span
                    style="color: #EDF5E1; font-size: 20px"><b><?php echo number_format($proiz["ukupnaCena"], 2, ',', '.'); ?></b>
                    RSD</span>
            </div>
            <div>
                <div>
                    <?php if($proiz['popust'] == null){ ?>
                    ...
                    <span id="popust">Popust: </span>
                    <span
                        style="color: #EDF5E1; font-size: 20px; margin-left: 5px;"><?php echo $proiz['popust']; ?>%</span>
                    <?php  
						}?>
                </div>
            </div>
        ...
        <div class="row">
            <div>
                ...
                <form method="post" action="korpa.php" class="form-inline">
                    <input type="hidden" name="idIzabranog" value="<?php echo $proiz['idProizvoda']; ?>">
                    <input type="number" name="izabranaKol" min="1" max="<?php echo $proiz['kolicina'] ?>"
